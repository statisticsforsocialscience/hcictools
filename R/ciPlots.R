...
#'
printCI <-
  function(d, titleScale = "Dependent Variable", delta, range = c(1,6), autorange = TRUE) {
    if (autorange) {
      range = c(0, max(c(d$upper[1:2],max(
        d$mean[1:2] + ceiling(d$upper[3])
      ))))
    }
    ...
    plot(
      NA, xlim = c(.5,3.7), ylim = range, bty = "l", xaxt = "n", xlab = "",ylab =
        paste(titleScale,"(mean and 95% CI)")
    )
    #insert Dat Points
...
createData <-
  function(lowerMean, upperMean, lowerSE, upperSE, diffLo, diffHigh, labelA, labelB, title) {
...
independentSampleMeansCI <- function(data1, data2, labelA = "A", labelB = "B", title = "Dependent Variable", range=c(1,0)) {
  ...
  ci <- t$conf.int
  temp <- createData(lowerMean = mean(data1), upperMean = mean(data2),
                     lowerSE = stderr(data1), upperSE = stderr(data2),
                     diffLo = ci[1], diffHigh = ci[2],
                     labelA = labelA, labelB = labelB, title=title)
